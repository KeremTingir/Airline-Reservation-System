// Reader thread'ini temsil eden sınıf
class Reader implements Runnable {
    ...
    @Override
    public void run() {
        // Reader'lar veritabanına senkronize bir şekilde erişir
        synchronized (db) {
            FlightSeat seat = db.getSeat(flightNumber, date);
            if (seat != null) {
                System.out.println("Reader checking seat for " + flightNumber + " on " + date + ": " + (seat.isReserved ? "Reserved" : "Available"));
            } else {
